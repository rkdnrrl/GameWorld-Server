...
const genreSchema = z.object({
  slug:      z.string().min(1).max(50).regex(/^[a-z0-9_-]+$/, 'slug는 소문자·숫자·-·_ 만 허용'),
...
router.get('/', async (req, res, next) => {
  try {
    const genres = await prisma.gameGenre.findMany({ orderBy: { sortOrder: 'asc' } });
    res.json({ genres });
  } catch (err) { next(err); }
});

/** POST /api/operator/genres — 장르 추가 */
operatorRouter.post('/', requireAuth, requireOperator, async (req, res, next) => {
  try {
    const data = genreSchema.parse(req.body);
    const genre = await prisma.gameGenre.create({ data: {
...
/** PATCH /api/operator/genres/:slug — 장르 수정 */
operatorRouter.patch('/:slug', requireAuth, requireOperator, async (req, res, next) => {
  try {
    const slug = req.params.slug;
    const data = patchSchema.parse(req.body);
    const genre = await prisma.gameGenre.update({ where: { slug }, data });
    res.json({ ok: true, genre });
  } catch (err) {
    if (err.name === 'ZodError') return res.status(400).json({ error: { message: err.issues?.[0]?.message || '입력 오류' } });
    if (err.code === 'P2025') return res.status(404).json({ error: { message: '장르를 찾을 수 없습니다.' } });
    next(err);
  }
});

/** DELETE /api/operator/genres/:slug — 장르 삭제 */
operatorRouter.delete('/:slug', requireAuth, requireOperator, async (req, res, next) => {
  try {
    const slug = req.params.slug;
    const count = await prisma.game.count({ where: { genre: slug } });
    if (count > 0) {
      return res.status(409).json({ error: { message: `${count}개 게임이 이 장르를 사용 중입니다. 게임의 장르를 먼저 변경해주세요.` } });
    }
    await prisma.gameGenre.delete({ where: { slug } });
    res.json({ ok: true });
  } catch (err) {
    if (err.code === 'P2025') return res.status(404).json({ error: { message: '장르를 찾을 수 없습니다.' } });
    next(err);
